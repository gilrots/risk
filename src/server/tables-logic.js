...
const argsMap = {
    ...
    b: 'bank',
    a: 'ace',
    t: 'table.calculated.aggregations',
    regEx: /{(.):(\d)}/g
};
    ...
    _.forEach(DB.types, type => {
        ...
        _.forEach(aggs, col => {
            _.forEach(col.func.aggregations, agg => {
                table.calculated.aggregations[agg.key] =_.reduce(result[type].data, (acc, dat) => {
            ...
            _.forEach(result[type].data, dat => {
                    const {stock, ace, bank} = dat;
                    const val = eval(col.func.exp);
                    table.calculated[col.key] = val;
                    dat.stock[col.key] = val;
            });
        });
